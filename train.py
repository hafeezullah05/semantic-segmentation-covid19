...
for e in tqdm(range(config.NUM_EPOCHS)):
	...
	# switch off autograd 
	# we can switch off the gradient computation with the help of torch.no_grad() and freeze the model weights
	with torch.no_grad():
		# set the model in evaluation mode  calling the eval() function
		unet.eval()
...
print("[INFO] total time taken to train the model: {:.2f}s".format(
    ...
    #plotting Training accuracy below
...
plt.savefig(config.PLOT_PATH) # we save the weights of our trained U-Net model with the help of the torch.save() function, 
